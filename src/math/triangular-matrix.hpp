...
namespace pinocchio
{
  ...
  namespace internal
  {
    ...
    struct TriangularMatrixMatrixProduct
    {
      ...
      static void run(const Eigen::MatrixBase<LhsMatrix> &lhs_mat,
                      const Eigen::MatrixBase<RhsMatrix> &rhs_vec,
                      const Eigen::MatrixBase<ResMat> &res)
      {
        res.const_cast_derived().col(0).noalias() = lhs_mat.derived().template triangularView<info>() * rhs_vec.derived();
      }
    };
    ...
    struct TriangularMatrixMatrixProduct<info, RhsMatrix, Scalar, false>
    {
      ...
      static void run(const Eigen::MatrixBase<LhsMatrix> &lhs_mat,
                      ...
                      const Eigen::MatrixBase<ResMat> &res)
      {
        res.const_cast_derived().noalias() = lhs_mat.derived().template triangularView<info>() * rhs_mat.derived();
      }
    };
  }
  ...
  inline void triangularMatrixMatrixProduct(const Eigen::MatrixBase<LhsMatrix> &lhs_mat,
                                            const Eigen::MatrixBase<RhsMatrix> &rhs_mat,
                                            const Eigen::MatrixBase<ResMat> &res)
  {
    internal::TriangularMatrixMatrixProduct<info, RhsMatrix>::run(lhs_mat.derived(),
                                                                  rhs_mat.derived(),
