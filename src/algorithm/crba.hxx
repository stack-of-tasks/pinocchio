...
namespace se3 
{
  ...
  bool isDescendant(const Model& model, const JointIndex j, const JointIndex root)
  {
  ...
  bool CRBAChecker::checkModel_impl( const Model& model ) const
  {
    ...
    for( JointIndex i=1; int(i)<model.njoint-1; ++i ) // no need to check joints 0 and N-1
      {
        JointIndex k=i+1;
        while(isDescendant(model,k,i)) ++k;
        for( ; int(k)<model.njoint; ++k ) 
          if( isDescendant(model,k,i) ) return false;
      }
    return true;
  }
