  ...
  namespace cholesky
  {
    ...
    namespace internal
    {
      ...
      struct Mv<Mat,MatRes,1>
      {
                        ...
                        const Eigen::MatrixBase<MatRes> & vout)
        {
          ...
          PINOCCHIO_UNUSED_VARIABLE(model);
          assert(model.check(data) && "data is not consistent with model.");
          PINOCCHIO_ASSERT_THROW_AT_RUNTIME(vin.size() == model.nv);
          PINOCCHIO_ASSERT_THROW_AT_RUNTIME(vout.size() == model.nv);
    ...
    namespace internal
    {
                   ...
                   const Eigen::MatrixBase<Mat> & v)
      {
        ...
        PINOCCHIO_UNUSED_VARIABLE(model);
        assert(model.check(data) && "data is not consistent with model.");
        PINOCCHIO_ASSERT_THROW_AT_RUNTIME(col < model.nv && col >= 0);
        PINOCCHIO_ASSERT_THROW_AT_RUNTIME(v.size() == model.nv);
    ...
    {
      PINOCCHIO_ASSERT_THROW_AT_RUNTIME(Minv.rows() == model.nv);
      PINOCCHIO_ASSERT_THROW_AT_RUNTIME(Minv.cols() == model.nv);
