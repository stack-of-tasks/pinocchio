...
BOOST_AUTO_TEST_CASE(test_correction_6D)
{
  ...
  RigidContactModel ci_LF(CONTACT_3D,LF_id,LOCAL);
  ci_LF.joint1_placement.setRandom();
  ...
  initContactDynamics(model,data,contact_models);
  const Eigen::VectorXd ddq0 = contactDynamics(model,data,q,v,tau,contact_models,contact_datas,mu);
  ...
  computeContactDynamicsDerivatives(model,data,contact_models,contact_datas,mu,
  ...
  for(Eigen::DenseIndex k = 0; k < model.nv; ++k)
  {
    ...
    Eigen::VectorXd ddq_plus = contactDynamics(model,data_fd,q_plus,v,tau,contact_models,contact_datas_fd,mu);
  ...
  for(Eigen::DenseIndex k = 0; k < model.nv; ++k)
  {
    ...
    Eigen::VectorXd ddq_plus = contactDynamics(model,data_fd,q,v_plus,tau,contact_models,contact_datas_fd,mu);
...
BOOST_AUTO_TEST_CASE(test_contact_dynamics_derivatives_LOCAL_3D_loop_closure_fd)
{
  using namespace Eigen;
  using namespace pinocchio;
  ...
  //RigidContactModel ci_closure (CONTACT_3D, LA_id, SE3::Random(),
  //                              RA_id, SE3::Random(), LOCAL);

  RigidContactModel ci_closure (CONTACT_3D, 0, SE3::Identity(),
                                RA_id, SE3::Random(), LOCAL);
  //RigidContactModel ci_closure (CONTACT_3D,
  //                              RA_id, SE3::Random(), 0, SE3::Identity(),  LOCAL);
  ...
  std::cerr<<"data.ddq_dq: "<<std::endl<<data.ddq_dq-ddq_partial_dq_fd<<std::endl;
  //std::cerr<<"data_fd.ddq_dq: "<<std::endl<<ddq_partial_dq_fd<<std::endl;
  ...
  std::cerr<<"data.ddq_dv: "<<std::endl<<data.ddq_dv-ddq_partial_dv_fd<<std::endl;
  //std::cerr<<"data_fd.ddq_dv: "<<std::endl<<ddq_partial_dv_fd<<std::endl;
...
BOOST_AUTO_TEST_CASE(test_contact_dynamics_derivatives_LOCAL_6D_loop_closure_fd)
{
  using namespace Eigen;
  using namespace pinocchio;
  ...
  //RigidContactModel ci_closure (CONTACT_6D, LA_id, SE3::Random(),
  //                              RA_id, SE3::Random(), LOCAL);

  RigidContactModel ci_closure (CONTACT_6D, 0, SE3::Identity(),
                                RA_id, SE3::Random(), LOCAL);
  //RigidContactModel ci_closure (CONTACT_3D,
  //                              RA_id, SE3::Random(), 0, SE3::Identity(),  LOCAL);
