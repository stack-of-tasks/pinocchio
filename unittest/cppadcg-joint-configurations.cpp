...
BOOST_AUTO_TEST_SUITE(BOOST_TEST_MODULE)
  ...
  BOOST_AUTO_TEST_CASE(test_integrate_code_generation)
  {
    ...
    std::vector<Eigen::VectorXd> results(2,Eigen::VectorXd::Zero(model.nq));
    CodeGenIntegrate<double> cg_integrate(model);
    cg_integrate.initLib();
    cg_integrate.loadLib();
    
    cg_integrate.evalFunction(qs,vs, results[0]);
    pinocchio::integrate(model, qs,vs,results[1]);
