...
int main(void) {
  ...
  ad_Q = ADVectorXs(nq); ad_Q = pinocchio::neutral(ad_model);
  ad_V = ADVectorXs(nv); ad_V.setOnes();
  ad_A = ADVectorXs(nv); ad_A.setZero();
  ...
  Eigen::VectorXd dv = Eigen::VectorXd(nv); dv.setOnes();
  ...
  Q = pinocchio::neutral(model); V.setOnes(); A.setZero();
  ...
  if (data.dtau_dv.isApprox(dtau_dv.reshaped(nv,nv).transpose())){
    std::cout << "Comparison from automatic differention to analytical derivatives succesful" << std::endl;
  }
  else {
    std::cout << "Comparison from automatic differention to analytical derivatives NOT succesful" << std::endl;
  }
}
