...
int main(int argc, const char ** argv)
{
  ...
  static PINOCCHIO_STD_VECTOR_WITH_EIGEN_ALLOCATOR(context::MatrixXs) col_major_square_matrices(NBT);
  static PINOCCHIO_STD_VECTOR_WITH_EIGEN_ALLOCATOR(context::RowMatrixXs) row_major_square_matrices(NBT);
  ...
  for(size_t i=0;i<NBT;++i)
  {
    ...
    col_major_square_matrices[i] = context::MatrixXs::Random(num_constraints,num_constraints) + 1.*context::MatrixXs::Identity(num_constraints,num_constraints);
  ...
  MatrixXd MJtJ_inv(model.nv+contact_chol_6D6D.constraintDim(),
