...
namespace pinocchio
{
  ...
  template<typename Scalar, int Options = context::Options> struct JointModelRevoluteUnboundedUnalignedTpl;
  typedef JointModelRevoluteUnboundedUnalignedTpl<context::Scalar> JointModelRevoluteUnboundedUnaligned;
  template<typename Scalar, int Options = context::Options> struct JointDataRevoluteUnboundedUnalignedTpl;
  ...
  template<typename Scalar, int Options = context::Options> struct JointModelHelicalUnalignedTpl;
  typedef JointModelHelicalUnalignedTpl<context::Scalar> JointModelHelicalUnaligned;
  
  template<typename Scalar, int Options = context::Options> struct JointDataHelicalUnalignedTpl;
  ...
  template<typename Scalar, int Options = context::Options> struct JointModelSphericalZYXTpl;
  typedef JointModelSphericalZYXTpl<context::Scalar> JointModelSphericalZYX;
  
  template<typename Scalar, int Options = context::Options> struct JointDataSphericalZYXTpl;
  ...
  template<typename Scalar, int Options = context::Options> struct JointModelPrismaticUnalignedTpl;
  ...
  template<typename Scalar, int Options = context::Options> struct JointModelUniversalTpl;
  typedef JointModelUniversalTpl<context::Scalar> JointModelUniversal;

  template<typename Scalar, int Options = context::Options> struct JointDataUniversalTpl;
  ...
  template<typename Scalar, int Options = context::Options> struct JointDataFreeFlyerTpl;
  typedef JointDataFreeFlyerTpl<context::Scalar> JointDataFreeFlyer;

  template<typename Scalar, int Options = context::Options> struct JointModelPlanarTpl;
  typedef JointModelPlanarTpl<context::Scalar> JointModelPlanar;
  
  template<typename Scalar, int Options = context::Options> struct JointDataPlanarTpl;
  typedef JointDataPlanarTpl<context::Scalar> JointDataPlanar;

  template<typename Scalar, int Options = context::Options> struct JointModelTranslationTpl;
  typedef JointModelTranslationTpl<context::Scalar> JointModelTranslation;
  
  template<typename Scalar, int Options = context::Options> struct JointDataTranslationTpl;
  typedef JointDataTranslationTpl<context::Scalar> JointDataTranslation;

  template<typename Scalar, int Options = context::Options> struct JointCollectionDefaultTpl;
