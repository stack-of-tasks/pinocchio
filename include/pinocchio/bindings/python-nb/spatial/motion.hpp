...
void exposeMotion(nb::module_ m)
{
  ...
  nb::class_<Motion>(
    ...
    .def_prop_rw(
      ...
      "Linear part of *this, corresponding to the linear velocity in case of a Spatial velocity.")
    .def_prop_rw(
      "angular", [](Self & self) { return Vector3(self.angular()); },
      [](Self & self, const Vector3 & w) { self.angular(w); },
      "Angular part of *this, corresponding to the angular velocity in case of a Spatial "
      "velocity.")
    .def_prop_rw(
    ...
    .def(
      "se3ActionInverse",
      [](const Self & self, const SE3 & M) { return self.se3ActionInverse(M); }, "M"_a,
    ...
    .def("__add__", [](const Motion & a, const Motion & b) { return Motion(a + b); }, nb::is_operator())
    .def("__sub__", [](const Motion & a, const Motion & b) { return Motion(a - b); }, nb::is_operator())
    .def("__neg__", [](const Motion & a) { return Motion(-a); }, nb::is_operator())
    .def("__iadd__", [](Motion & a, const Motion & b) { return Motion(a += b); }, nb::is_operator())
    .def("__isub__", [](Motion & a, const Motion & b) { return Motion(a -= b); }, nb::is_operator())
    .def("__mul__", [](const Motion & m, Scalar s) { return Motion(m * s); }, nb::is_operator())
    .def("__rmul__", [](const Motion & m, Scalar s) { return Motion(s * m); }, nb::is_operator())
    .def("__truediv__", [](const Motion & m, Scalar s) { return Motion(m / s); }, nb::is_operator())
    .def("__xor__", [](const Motion & a, const Motion & b) { return Motion(a.cross(b)); }, nb::is_operator())
    // Comparison
    .def(
      "isApprox",
    ...
    .def(
      ...
      "given by prec.")
    .def("__eq__", [](const Motion & a, const Motion & b) { return a == b; }, nb::is_operator())
    .def("__ne__", [](const Motion & a, const Motion & b) { return a != b; }, nb::is_operator())
